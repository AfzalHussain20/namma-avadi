...
export default async function MembersPage({
  ...
  const q = first(sp.q)
  const ward = first(sp.ward) ? parseInt(first(sp.ward), 10) : null
  ...
  const [{ members, total, pages }, wards] = await Promise.all([
    listMembers({ q, ward, from, to, page, perPage: PER_PAGE }),
    getWards(),
  ])
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-500">
          Browse and filter all registered TVK members in Avadi.
        </p>
      </div>

      <MemberFilters wards={wards} />

      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          <p className="text-sm font-medium text-slate-500">
            {total.toLocaleString('en-IN')} member{total === 1 ? '' : 's'} found
          </p>
          {total > 0 && (
        ...
        {members.length === 0 ? (
          <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
            ...
            <p className="mt-4 text-base font-semibold text-slate-900">No members found</p>
            <p className="mt-2 max-w-sm text-sm text-slate-500">
              Try adjusting your search or filters. New members can be added from the registration page.
            </p>
          </div>
        ) : (
          <>
                  ...
                  {members.map((m) => {
                    ...
                    return (
                      <tr key={m.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60">
                        ...
                        <td className="td">
                          <span className="badge badge-blue">Ward {m.ward_number}</span>
                        </td>
